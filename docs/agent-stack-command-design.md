...
Provide collision-resistant commands for launching named Copilot, Claude,
and Codex sessions:

```text
agent-stack copilot <name>
agent-stack claude <name>
agent-stack codex <name>
```
...
collide with existing system and user commands. Expose bounded Screen Sharing
as `agent-screen` instead of claiming the existing `ss` command.
...
backend, renames the Screen Sharing command, and removes previously owned
aliases during upgrade and uninstall.

## Non-goals

- Preserve `ca`, `cc`, `co`, or `ss` as supported compatibility commands.
- Install Copilot, Claude, or Codex itself.
- Change existing workspace layouts, live tmux session names, session rotation,
  mailbox behavior, or permission defaults. New Copilot sessions retain the
  established unprefixed namespace; matching pre-release `copilot-<name>`
  sessions remain attachable without creating new prefixed sessions.
...
`agent-stack` requires a backend and session name:

```text
agent-stack <copilot|claude|codex> <name>
```
...
`agent-stack -h|--help`, `agent-stack <backend> -h|--help`, and each long
...
|---|---|---|
| Copilot | `$COPILOT_WORKSPACE_BASE/agent-<name>` | `<name>` |
| Claude | `$CLAUDE_WORKSPACE_BASE/agent-<name>` | `claude-<name>` |
| Codex | `$CODEX_WORKSPACE_BASE/agent-<name>` | `codex-<name>` |

Claude and Codex use explicit backend prefixes. Copilot retains its established
unprefixed `<name>` namespace so mailbox identity and existing snippets remain
stable. A short-lived pre-release created `copilot-<name>` sessions; the wrapper
may reattach to one only when no unprefixed session exists and its physical
session path matches the expected Copilot workspace. It never creates a new
prefixed Copilot session.
...
a symlink to this checkout's agent wrapper. It removes `ss` only when it points
to this checkout's former `bin/ss` path. Foreign paths remain untouched. The
...
- `agent-screen` is the only installed Screen Sharing command; `ss` is never
...
| Existing unprefixed Copilot session has another physical workspace | Refuse to attach and report both paths |
...
2. inspect `agent-stack`, `copilot-agent`, `claude-agent`, `codex-agent`,
   `agent-screen`, `ca`, `cc`, `co`, `ss`, `remote-agent`, and `remote-screen`,
   and abort before mutation only
   when a foreign
...
5. run its installer, which recreates its supported links and skips foreign
   short-command paths with a warning;
...
- new sessions use the established unprefixed Copilot namespace and the
  `claude-` or `codex-` namespaces;
- matching pre-release `copilot-<name>` sessions remain attachable but are
  never newly created;
...
- all five managed links are planned and installed;
...
to this checkout among `agent-stack`, `copilot-agent`, `claude-agent`,
`codex-agent`, `agent-screen`, `ca`, `cc`, `co`, `ss`, `remote-agent`, and
`remote-screen` are removed and that
`--commands-only` leaves every non-command artifact untouched.
...
- `agent-stack copilot <name>`, `agent-stack claude <name>`, and
  `agent-stack codex <name>` launch or reattach to the correct named session.
- Omitting `<name>` prints usage and exits 2.
- The three long aliases behave identically.
- The installer no longer installs `ca`, `cc`, `co`, or `ss` and safely removes
  only retired commands it owns.
- `agent-screen on|off|status` replaces the retired `ss` command.
